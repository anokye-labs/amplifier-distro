...
class TestRestartServerSupervisorDetection:
    """Verify _restart_server() supervisor detection and RuntimeError handling."""

    @patch("amplifier_distro.server.watchdog.stop_process")
    @patch("amplifier_distro.server.watchdog.pid_file_path")
    def test_systemd_invocation_id_exits_1(
        self,
        mock_pid_file_path: MagicMock,
        mock_stop_process: MagicMock,
        monkeypatch: object,
    ) -> None:
        """When INVOCATION_ID is set, stop server and exit with code 1."""
        import pytest

        from amplifier_distro.server.watchdog import _restart_server

        monkeypatch.setenv("INVOCATION_ID", "some-systemd-id")
        monkeypatch.delenv("LAUNCHD_JOB_NAME", raising=False)
    ...
    @patch("amplifier_distro.server.watchdog.stop_process")
    @patch("amplifier_distro.server.watchdog.pid_file_path")
    def test_launchd_job_name_exits_1(
        self,
        mock_pid_file_path: MagicMock,
        mock_stop_process: MagicMock,
        monkeypatch: object,
    ) -> None:
        """When LAUNCHD_JOB_NAME is set, stop server and exit with code 1."""
        import pytest

        from amplifier_distro.server.watchdog import _restart_server

        monkeypatch.delenv("INVOCATION_ID", raising=False)
        monkeypatch.setenv("LAUNCHD_JOB_NAME", "com.example.amp-distro")
    ...
    @patch("amplifier_distro.server.watchdog.is_running")
    @patch("amplifier_distro.server.watchdog.pid_file_path")
    def test_runtime_error_from_daemonize_is_caught(
        self,
        mock_pid_file_path: MagicMock,
        ...
        """RuntimeError from daemonize() is caught; function returns silently."""
        from amplifier_distro.server.watchdog import _restart_server
    ...
    @patch("amplifier_distro.server.watchdog.is_running")
    @patch("amplifier_distro.server.watchdog.pid_file_path")
    def test_standalone_restart_succeeds(
        self,
        mock_pid_file_path: MagicMock,
        mock_is_running: MagicMock,
        mock_daemonize: MagicMock,
        ...
        """Standalone (no supervisor) path restarts server normally."""
        from amplifier_distro.server.watchdog import _restart_server
